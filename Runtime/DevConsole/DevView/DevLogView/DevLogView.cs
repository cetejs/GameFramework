...
namespace GameFramework.DevConsoleService
{
    internal partial class DevLogView : MonoBehaviour
    {
        ...
        private readonly CacheLinkedList<LogInfo> logInfos = new CacheLinkedList<LogInfo>();
        ...
        private void Awake()
        {
            maxLogCount = DevConsoleConfig.Get().maxLogCount;
        ...
        private bool CheckLogOverflow()
        {
            if (logInfos.Count > maxLogCount)
            {
                ...
                {
                    ReferencePool.Release(logInfos.First.Value);
        ...
        private void OnMessageReceived(string condition, string stackTrace, LogType logType)
        {
            ...
            if (logInfo == null)
            {
                logInfo = ReferencePool.Get<LogInfo>();
