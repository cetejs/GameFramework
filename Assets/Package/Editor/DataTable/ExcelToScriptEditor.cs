...
namespace GameFramework
{
    internal sealed class ExcelToScriptEditor
    {
        ...
        public async void ExcelToCs(DataTable dataTable, string fileName)
        {
            await Task.Run(() =>
            {
                ...
                string tempTextPath = string.Concat(EditorFileUtils.GetPackagePah(), "/Editor/DataTable/TemplateTable.txt");
                ...
                stringBuilder.Replace("#Namespace#", config.scriptNamespace);
                ...
                string fullPath = Path.Combine(Path.GetFullPath(config.scriptBuildPath), fileName);
