...
namespace GameFramework.Utils
{
    public static class TransformUtils
    {
        ...
        public static void AdjustAnchor(this RectTransform rectTransform, AnchorLeftType leftType, AnchorTopType topType)
        {
            if (leftType == AnchorLeftType.Stretch || topType == AnchorTopType.Stretch)
            {
                if (leftType != AnchorLeftType.Stretch)
                {
                    ...
                    rectTransform.anchorMin = new Vector2(0.0f, leftValue);
                    rectTransform.anchorMax = new Vector2(1.0f, leftValue);
                ...
                else if (topType != AnchorTopType.Stretch)
                {
                    ...
                    rectTransform.anchorMin = new Vector2(topValue, 0.0f);
                    rectTransform.anchorMax = new Vector2(topValue, 1.0f);
        ...
        {
            if (screenPoint.x < 0.0f || screenPoint.x > Screen.width || screenPoint.y < 0.0f || screenPoint.y > Screen.height)
            {
                return false;
            }
