...
namespace GameFramework
{
    ...
    [InitializeOnLoad]
    internal static class BoneRendererUtils
    {
        ...
        private static List<BoneRenderer> s_BoneRendererComponents = new List<BoneRenderer>();
        ...
        private static void DrawSkeletons(SceneView sceneview)
        {
            ...
            for (int i = 0; i < s_BoneRendererComponents.Count; i++)
            {
                BoneRenderer boneRenderer = s_BoneRendererComponents[i];
                ...
                if (boneRenderer.drawBones)
                {
                    ...
                    Color color = boneRenderer.boneColor;

                    for (int j = 0; j < boneRenderer.bones.Length; j++)
                    {
                        BoneRenderer.TransformPair bone = boneRenderer.bones[j];
                    ...
                    for (int k = 0; k < boneRenderer.tips.Length; k++)
                    {
                        Transform tip = boneRenderer.tips[k];
                        if (tip == null)
                            continue;
                ...
                if (boneRenderer.drawTripods)
                {
                    ...
                    for (int j = 0; j < boneRenderer.transforms.Length; j++)
                    {
                        float tripodSize = 1f;
                        Transform transform = boneRenderer.transforms[j];
                        if (transform == null)
                            continue;

                        Vector3 position = transform.position;
                        Vector3 xAxis = position + transform.rotation * Vector3.right * size * tripodSize;
                        Vector3 yAxis = position + transform.rotation * Vector3.up * size * tripodSize;
                        Vector3 zAxis = position + transform.rotation * Vector3.forward * size * tripodSize;
        ...
        {
            foreach (BoneRenderer boneRenderer in s_BoneRendererComponents)
            {
